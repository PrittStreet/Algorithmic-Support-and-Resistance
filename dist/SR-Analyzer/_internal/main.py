...
# ── Path resolution (dev vs PyInstaller frozen bundle) ─────────────────────────
import sys as _sys

def _get_base_dir() -> Path:
    """Mutable data dir — next to exe when frozen, __file__ parent in dev."""
    if getattr(_sys, 'frozen', False):
        return Path(_sys.executable).parent
    return Path(__file__).parent

def _get_bundle_dir() -> Path:
    """Read-only bundle dir — sys._MEIPASS when frozen, __file__ parent in dev."""
    if getattr(_sys, 'frozen', False):
        return Path(_sys._MEIPASS)
    return Path(__file__).parent

# ── SQLite ─────────────────────────────────────────────────────────────────────
DB_PATH = _get_base_dir() / "sr_data.db"
...
def _download_batch_blocking(tickers: list, period: str, interval: str, max_retries: int = 3) -> dict:
    global _yahoo_request_count
    for attempt in range(max_retries):
        try:
            _yahoo_request_count += 1
...
async def get_stats():
    ...
    return {
        ...
        "yahoo_requests": _yahoo_request_count,
        "cache_entries": len(_ohlcv_cache),
    }
...
async def rename_preset(preset_id: str, body: PresetPatch):
    await _db_execute("UPDATE presets SET name=? WHERE id=?", (body.name.strip(), preset_id))
    return {"ok": True}
...
# ── Serve React build ─────────────────────────────────────────────────────────
_STATIC = _get_bundle_dir() / "static"
if _STATIC.exists():
